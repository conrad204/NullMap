  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const view: View = {
        width,
        height,
        toX: (x) => width / 2 + (x - midX) * scale,
        toY: (y) => height / 2 - (y - midY) * scale,
      };
      ...
      for (const point of points) {
        ...
        ctx.beginPath();
        ctx.arc(view.toX(point.x), view.toY(point.y), 1.3, 0, Math.PI * 2);
        ctx.fill();
      }

      for (const region of regions) {
        ctx.beginPath();
        ctx.arc(view.toX(region.x), view.toY(region.y), 4.5, 0, Math.PI * 2);
