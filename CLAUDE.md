...
- `inconclusive` is not a finding: every such verdict carries an `inconclusive_reason` (`statistics.INCONCLUSIVE_REASONS`; only `wide_interval` describes the study, the rest describe what could be read). `BUCKET_SCRIPT` emits `inconclusive:<reason>` and `aggregate` folds it into `bucketCounts` plus `inconclusiveReasons`; the UI keeps it out of the verdict bar (`BAR_VERDICTS`) and shows the count with its reasons instead. The bar itself shows three answers, not five buckets (`BAR_GROUPS`: made a difference / made no difference / no answer); the confirmed-versus-claimed and stopped-versus-unreported splits survive only as the count line under each answer and on each study row, so the backend taxonomy is unchanged.
