...
`SearchPipeline._search` runs: `llm.parse` (PICO + SESOI, falls back to raw idea) → `repo.retrieve` (hybrid BM25 + kNN fused by RRF, or BM25-only when embeddings are off) → `expand` (top 3 indexed reviews' `referenced_works`, re-scored by cosine against the query vector, screened by population; only already-indexed works, missing ones are reported as warnings) → `registry_sweep` → `link_studies` at query time (newly discovered links are persisted) → `screen` (one small-model yes/no relevance pass per 40 retrieved studies, up to `SCREEN_LIMIT`, run before extraction so irrelevant keyword matches are never paid for; fails open with a warning; when it covered the whole match set, `recount` replaces the keyword-match bucket counts, otherwise a warning says the counts are unscreened) → `extract_one` for top `extraction_limit` openalex/merged studies (per-document `asyncio.Lock`; cache is fresh when the stored `extraction_version` equals `EXTRACTION_CACHE_VERSION`, so bump that setting whenever the extraction prompt or evidence-line format changes; if the paper has a `pmcid`, `fulltext.py` fetches Europe PMC JATS XML and flattens abstract + primary-outcome methods sentences + table rows + results prose into verbatim lines, otherwise the abstract is split into sentences; quoted evidence must be a verbatim line or the extraction is `rejected`, except that an unsupported total `n`, `p_value`, `reported_result` or arm-level summary is dropped on its own (arm summaries as a whole group) because losing those can only remove information; numbers written in words or space-grouped digits count as verbatim; `fulltext_status` of `used`/`unavailable` stops re-fetching, and an abstract-only cache entry is upgraded to full text exactly once; a failed re-extraction keeps the previous facts as `retained_verified`) → `repo.aggregate` (bucket counts, year histogram, `significant_text` null terms, spin rate over the full match set) → `group_outcomes` (LLM comparability judgment over studies that already have a standard error) → `analyze_studies` (`statistics.py`, pure Python) → `effect_trend` → optional `llm.narrate` only when a compatible numeric pool exists.

Optional **pre-search corpus filters** (`models.SearchFilters`: `yearFrom`, `yearTo`, `minCitations`, `maxCitations`, each independently optional) narrow the corpus before anything else runs. `repository.filter_clauses` turns them into mandatory `filter` clauses inside `lexical_query`, so retrieval, the registry sweep and `aggregate` share one match set and the expanded-reference escape hatch (`should: ids`) cannot bypass them; `screen_population` reapplies them to review references, which never pass through the lexical query. Dates use `publication_date` (for registry rows, the primary completion date), so a record without one cannot satisfy a year bound. **Registry rows (`ctgov`, `merged`) are exempt from citation bounds**: ingestion writes `cited_by_count: 0` for every trial, so a citation floor would delete exactly the terminated and never-reported trials the tool exists to surface. The exemption is never silent: `registry_citation_exemption_query` counts the registry rows the bound would have removed (`registryExempted`), the warning names that number, and `lib/filters.registryExemptionNotice` states it in the report header beside the active filters — but only when a citation bound actually applied and registry rows were actually kept (an exempted count of 0, or no count and no registry row on the page, shows nothing). A filtered search emits warnings and a `filters` payload (`description`, `registryCitationExemption`, `matchedBeforeFilters`, `excluded`) where `matchedBeforeFilters` is one `repository.count_studies` call over the same match-set predicate without the bounds; when that count fails the report says the exclusion could not be counted instead of implying nothing was removed. An empty filter object is not a filter: `filters` stays null and nothing warns.
...
Routes are `/health`, `/ready`, `/search`, `/search/stream`, `/studies/{id}`, `/novelty`, `/map`, `/concepts/search`. Every route is registered twice: bare (`/search`) and prefixed (`/api/search`, hidden from schema). Vite strips `/api` in dev; the built `dist/` is served by FastAPI at `/` when present (`FRONTEND_DIST`). `/search/stream` is SSE over POST with `progress`, `result`, `error` events and `: keepalive` comments; the frontend falls back to plain `/search` on 404/405.

### Gap map (`backend/app/gapmap.py`, `gapmap_service.py`)

Backend only on `main`: the Map tab that drew it (`MapPanel`, `MapCanvas`, `lib/regions.ts`, `fetchMap`, the `GapMap` types) lives on the `map-tab` branch. `POST /map` describes the corpus rather than a query: `repository.sample_embedded` takes a deterministic `random_score` sample of embedded studies, `build_regions` clusters them with a dependency-free spherical k-means, and each region is labeled by *why* nothing new is there — `null_saturated`, `dark`, `contested`, `active`, `unread`, `thin` (`label_region`, reviews excluded from attempts). `unread` is the fallback rather than `active`: on the live index most regions carry no readable outcome at all, and calling those active would claim a literature nobody read. `find_gaps` interpolates: the midpoint between two neighboring centroids is a gap when the two regions' own members almost never prefer the midpoint to either parent, and no third centroid sits in the band; adjacency is the top decile of pairwise centroid cosine (`neighbour_threshold`), not a constant, because "close" depends on the embedding model and corpus breadth. Whether a band is empty depends on `GAPMAP_REGIONS`, which is a resolution setting: measured on the live index, 24 regions yield none and 80 yield a handful. `place` returns the cosine to the nearest indexed paper as `redundancy` — a redundancy statistic, never a probability of novelty. `calibrate` rebuilds the map before a cutoff year and reports what later papers did per historical label; it is calibration, not prediction. The service caches one map, strips centroids from the payload, and warns whenever the sample is smaller than the embedded corpus.

### Concept-vector search (`backend/app/concepts.py`)

`POST /concepts/search` is the document-level analogue of `king − man + woman`: each supplied positive and negative concept is embedded once, `gapmap.combine` turns them into one unit direction, and `repository.knn_studies` returns the nearest studies to it. The module is deliberately independent of the search pipeline and the gap map so other features can reuse `resolve` for the combined vector alone. Every match carries its cosine to each supplied concept, in the order supplied, so the UI can show what pulled a paper in and what should have pushed it away; Elasticsearch's `(1 + cos) / 2` score is converted back to a cosine, and nothing else in the payload is a score. Disabled or unavailable embeddings are a 503 and a combination that cancels out is a 422, never an unranked or invented result.

The same arithmetic also steers the ordinary search, which is all the UI uses it for: `SearchRequest.concepts` (`models.ConceptSteer`, up to 8 terms a side) is embedded by `pipeline.aim`, and `concepts.aim` treats the question's own vector as one more positive term before `repo.retrieve`. The lexical query is untouched, so tags change the order of the question's matches and never the match set; embeddings off, an embedding failure and a canceling set each warn and fall back to the unsteered vector. `retrieval.concepts` echoes the tags (null when there were none) so the report header can say what reordered it (`ConceptSteerNote` in `ResultsView`). The UI has one box: `concepts.parseLine` reads the typed line as the question plus any signed terms trailing it ("…in adults? +blood pressure −stroke"), a sign counting only when it opens a term and the term follows it immediately, so hyphenated words and mid-sentence dashes stay prose. The line is the only source of truth, so nothing reads it back as a second control: `concepts.tagRanges`/`tagSegments` give the same reading as character ranges and `Composer` colors the tags inside the box, drawing a mirror of the line behind a transparent-text textarea that stays the real input.
...
- `inconclusive` is not a finding: every such verdict carries an `inconclusive_reason` (`statistics.INCONCLUSIVE_REASONS`; only `wide_interval` describes the study, the rest describe what could be read). `BUCKET_SCRIPT` emits `inconclusive:<reason>` and `aggregate` folds it into `bucketCounts` plus `inconclusiveReasons`; the UI keeps it out of the verdict bar (`BAR_VERDICTS`) and shows the count with its reasons instead. The bar itself shows four answers, not five buckets (`BAR_GROUPS`: favored the intervention / favored the comparator / made no difference / no answer); a group is a predicate over a study plus a total over `bucketCounts` + `effectDirections`, so the bar, the legend and the study-list filter share one definition and a group can be finer than a bucket. The confirmed-versus-claimed and stopped-versus-unreported splits survive only as the count line under each answer and on each study row, and an `effect` of unstated direction is counted beside the bar (`UNSTATED_DIRECTION_GROUP`), so the backend taxonomy is unchanged.
- A text-level verdict needs a comparison group: `reported_result` is honored only with a quoted `has_control` (or a registry row), and a paper that was read (`extraction_status: verified`) without one gets no verdict from the lexicon either. This keeps case reports out of `effect`; `BUCKET_SCRIPT` mirrors it.
