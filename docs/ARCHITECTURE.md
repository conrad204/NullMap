...
5. **Extract**: for the top `EXTRACTION_LIMIT` paper hits (default 100) without `extracted_at`, one structured-output call each (parallel, capped by `LLM_CONCURRENCY`): design, N per arm, primary outcome, effect type, estimate, CI, p, whether the primary outcome was met, and the verbatim sentence supporting each number. Reject any extraction whose quoted sentence is not a substring of the abstract. Write back to ES.

   **Amendment, 2026-09-19 evening:** abstracts under-report nulls, especially secondary and spun primary outcomes. When a paper has a PMCID, the extract step first fetches Europe PMC JATS XML (keyless) and flattens it into verbatim lines: abstract sentences, methods sentences that name the prespecified primary outcome or power calculation, results table rows as `[Table label] cell | cell`, then results prose, capped at `FULLTEXT_MAX_LINES`. Introduction, discussion, conclusions and back matter are excluded so a restated secondary finding cannot pose as the primary. The same sentence-index protocol and verbatim-quote rejection apply. The cache records `extraction_source` (`abstract` or `full_text`) and `fulltext_status` (`used` or `unavailable`) so an abstract-only cache is upgraded exactly once and a missing full text is never re-fetched. Papers outside PubMed Central stay abstract-only.
...
- **File drawer, measured rather than inferred:** the share of registered, completed trials on this question with no results anywhere. It is the primary reporting-bias signal because it needs no pool at all, and it is stated as a caveat on the pooled estimate.
- **Funnel asymmetry:** Egger's regression of each study's estimate/SE on its precision 1/SE, run per pool and only from 10 studies of differing precision (below that the test has no power, and the registry ratio carries the argument instead). Report the intercept with its CI, t and p, and draw the funnel. Asymmetry is also produced by heterogeneity, study quality and chance, so it is reported as a caveat and never used to adjust μ.
